...
object Kleislis {
  ...
  val func3K: Kleisli[Option, Int, String] = func2K andThen func1K // first multiplies, then add sting

  // convenience
  val multiply = func2K.map(_ * 2) // x => Option(...).map(_ * 2)
  val chain = func2K.flatMap { x => println(x); func1K } // the same value is passed to all
