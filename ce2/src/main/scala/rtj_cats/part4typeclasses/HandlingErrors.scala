package rtj_cats.part4typeclasses
...
object HandlingErrors {
  ...
  /** this is no more than the way to provide more semantics for generic Left & Right */
  ...
  /** here is total function, compiler will warn */
  def whateverTotal(v: Int)(f: Int => String) = f(v)

  val residualHandler: PartialFunction[Int, String] = { case _ =>
    "Another"
  }

//  def whateverPartial(v: Int)(f: PartialFunction[Int, String]) = f.orElse(residualHandler)(v)
  /** here is partial function, compiler will NOT warn */
  def whateverPartial(v: Int)(f: PartialFunction[Int, String]) = f(v)
