...
object ContextExtractor {
  ...
  private def toEither[A](a: Option[A]): Either[Unit, A] = a.toRight(())
  ...
  def option[F[_]: Functor, A](f: Request[F] => F[Option[A]]): Extractor[F, Unit, A] = f.andThen(_.map(toEither))

  def option[F[_]: Applicative, A](f: Request[F] => Option[A]): Extractor[F, Unit, A] = either(f.andThen(toEither))
