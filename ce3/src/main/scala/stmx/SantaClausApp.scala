...
class SantaClausProblem(val stm: STM[IO]) {
  import stm.TVar
  import stm.Txn
  ...
  object Gate {
    def of(capacity: Int) =
      TVar.of(0).map(x => new Gate(capacity, x) {})
  ...
  def choose[A](choices: NonEmptyList[(Txn[A], A => IO[Unit])]): IO[Unit] = {
    def actions: NonEmptyList[Txn[IO[Unit]]] = choices.map { case (guard, rhs) =>
      for {
        value <- guard
      } yield rhs(value)
    }
  ...
  def santa(elfGroup: Group, reinGroup: Group): IO[Unit] = {
    ...
    for {
      ...
      _ <- choose[(Gate, Gate)](
             NonEmptyList.of(
               (reinGroup.await, { (gg: (Gate, Gate)) => run("deliver toys", gg) }),
               (elfGroup.await, { (gg: (Gate, Gate)) => run("meet in study", gg) })
             )
           )
    } yield ()
  }
...
object SantaClausApp extends IOApp.Simple {

  override def run: IO[Unit] =
    STM
      .runtime[IO]
      .flatMap(stm => new SantaClausProblem(stm).run)
