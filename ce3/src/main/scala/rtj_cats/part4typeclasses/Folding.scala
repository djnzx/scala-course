...
object Folding {
  ...
  def main(as: Array[String]): Unit = {
    // folding to the value
    val x: Int = Foldable[Option].foldLeft(Option(11), 22)(_ + _)

    val xs = List(1, 2, 3)
    val ys = List(11, 12, 13)

    /** using ys as the initial value, scanning xs from right, attaching them to the head of ys */
    val zs = xs.foldRight(ys)((a, as) => a :: as)
    pprint.pprintln(zs)
  }
}
