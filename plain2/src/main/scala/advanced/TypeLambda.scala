...
object TypeLambda extends App {
  ...
  /** but we have G[_, _] with one fixed type, for example to Int => G[Int, _] we have two ways to solve the problem:
    *
    *   1. type alias trait G[_, _] type G1[A] = G[Int, A] and use it
    *
    * 2 type lambda
    */
  // approach 1.
...
//  def makeFunctor[A]: Functor[Map[Int, A]] = new Functor[Map[Int, A]]] {}
  ...
  /** we set 1st type parameter here and Functor has no information about it Functor will operate with, Map[Int, _] as
    * F[_]
    */
  val fun1: Functor[
    ({
