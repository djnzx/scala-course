...
object Fps010Recursion1 extends App {
  ...
  val list: Seq[Int] = List(10,20,30)

  // recursive but non TR
  def sum[A: Numeric](list: Seq[A]): A = list match {
  ...
  def sum2[A: Numeric](list: Seq[A]): A = sumr(list, implicitly[Numeric[A]].zero)
  def sumr[A: Numeric](list: Seq[A], acc: A): A = list match {
    case Nil => acc
