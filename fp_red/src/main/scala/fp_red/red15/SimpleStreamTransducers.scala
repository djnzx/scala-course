...
object SimpleStreamTransducers {
  ...
  /** We now introduce a type, `Process`, representing pure, single-input stream transducers. It can be in of three
    * states, it can be:
  ...
  sealed trait Process[A, B] {
    ...
    /** this is a driver (interpreter) of our stream A `Process[I,O]` can be used to transform `Stream[I]` to a
      * `Stream[O]`
      */
    ...
    /** `Process` definitions can often be expressed without explicit recursion, by repeating some simpler `Process`
      * forever
      */
    ...
    /** `Process` can be thought of as a sequence of values of type `O` and many of the operations that would be defined
      * for `List[O]` can be defined for `Process[I,O]`, for instance `map`, `++` and `flatMap`. The definitions are
      * analogous.
      */
    ...
    /** As an example of `repeat`, see `Process.filter`. We define a convenience function here for composing this
      * `Process` with a `Process` that filters the output type `O`.
      */
  ...
  object Process {
    ...
    /** We can convert any function `f: I => O` to a `Process[I,O]`. We simply `Await`, then `Emit` the value received,
      * transformed by `f`. We terminate stream after 1st element
      */
    ...
    /** Here's a typical `Process` definition that requires tracking some piece of state (in this case, the running
      * total):
      */
    ...
    /** Here's one implementation, with three stages - we map all inputs to 1.0, compute a running sum, then finally
      * convert the output back to `Int`. The three stages will be interleaved - as soon as the first element is
      * examined, it will be converted to 1.0, then added to the running total, and then this running total will be
      * converted back to `Int`, then the `Process` will examine the next element, and so on.
      */
    ...
    /** every new item ot type A potentially can produce 0 or more elements of type B to handle that, probably we need
      * emit items recursively
      */
    ...
    /** to make it lazy even in term of one chunk, we emit sequence of elements with type (B) and in the end we emit the
      * state (S)
      *
      * @param s
      *   - initial state
    ...
    def loopsst2[S, A, B](
        s: S,
      )(f: (S, Option[A]) => Stream[Either[S, B]],
      )(rf: S => Stream[B] = (_: S) => Stream.empty,
      ): Process[A, B] =
      await(
        { i: A =>
          f(s, Some(i)) match {
...
//          }
    ...
    /** Exercise 7: Can you think of a generic combinator that would allow for the definition of `mean` in terms of
      * `sum` and `count`?
      *
      * Yes, it is `zip`, which feeds the same input to two processes. The implementation is a bit tricky, as we have to
      * make sure that input gets fed to both `p1` and `p2`.
      */
    ...
    /** Exercise 8: Implement `exists`
      *
      * We choose to emit all intermediate values, and not halt. See `existsResult` below for a trimmed version.
      */
