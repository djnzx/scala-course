...
object P27 {
  ...
  implicit class ListSyntax[A](private val me: List[A]) extends AnyVal {
    def --(another: List[A]): List[A] = me.filterNot(another.contains)
  }
  ...
  //                                             groups
  //                                                  group
  def group[A](ns: List[Int], la: List[A]): List[List[List[A]]] = {
  ...
  def groupA[A](ns: List[Int], la: List[A]): List[List[List[A]]] = ns match {
    case Nil => List(Nil)
    case n :: ns =>
      for {
        c <- combinations(n, la)
