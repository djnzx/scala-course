...
object B010FriendsPairingCombinations {
  ...
  def allCombN[A](data: List[A], n: Int): List[List[List[A]]] = {
    val N = data.size
  ...
  def allComb12[A](data: List[A]): List[List[List[A]]] = {
    val N = data.size
    if (N == 0) return List(Nil)
...
//      .foldLeft(List(List.empty[A]))(_:::_)     // I have no idea why foldLeft DOESN'T WORK
      .flatMap { la =>                            // 2. take n-th group (1/2 - doesn't matter)
        ...
        val b = a.map { lla => la :: lla }        // 4. attach the taken group from step2 
        b
      }
    // @formatter:on
  }
