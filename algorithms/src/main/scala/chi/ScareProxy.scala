...
/** DISCLAIMER. this is only an idea
  *
  *   - Async and Concurrent stuff haven't been taken into account.
  *   - we assume that spawning an instance and terminating them have zero overhead. Actually, we need to use more
  *     sophisticated algorithm and bring to the table something like moving average to start our instances in advance
